...
| 文件 | 目录 | 产出人 | 说明 |
|------|------|--------|------|
| [report2article — 研究报告编排器 skill](engineering/skills/report2article/SKILL.md) | engineering/skills | 小帅 | 「研究报告→读者向文章」编排器 skill（定位=编排器非写手，红线=只重组/排序/串联/起标题，不对报告内容增删改）。三解法：①主标题 27 词夸张黑名单+客观/准确/清晰②子标题≤20字·禁搬对象全名/整句结论/机械命名③「为什么读→是什么→怎么演变→意味着什么」四段逻辑骨架。含信息零丢失 5 维强制闸 + 超长报告分块编排/落盘/销账 X==N 协议。含 SKILL.md+4 references（title-rules/logic-skeleton/info-checklist-template/long-report-protocol）。已发 ClawHub（report2article@1.0.0）+ 接入 6 个周报 |
