...
```
旧：listActions() / getCapabilities() / getToolSchema()
新：describeMessageTool(...)
```
...
```
旧：nano-banana-pro 技能包
新：agents.defaults.imageGenerationModel.primary: "google/gemini-3-pro-image-preview"
```
...
飞书用户 → DM / 话题会话
             ↓
  resolveSessionBinding() → 绑定到 ACP 或子 Agent 会话
             ↓
  Agent 执行完成 → 结果投递回原始飞书会话
```
...
/reasoning stream → 开启思维链流式输出
                     ↓
Agent 思考中... → onReasoningStream → 卡片实时更新 blockquote
                     ↓
思考完成 → onReasoningEnd → 正式回复替换卡片
```
